...
static int start_camera(void)
{
	...
	printk("dual-tflm: ISP input selected %ux%u pitch=%u\n",
	...
	for (size_t i = 0; i < ARRAY_SIZE(camera_buffers); ++i) {
		...
		if (camera_buffers[i] == NULL) return -ENOMEM;
		rc = video_enqueue(camera_dev, VIDEO_EP_OUT, camera_buffers[i]);
		if (rc != 0) return rc;
	}
	camera_started = true;
	printk("dual-tflm: Zephyr MT9M114 ready %ux%u pitch=%u buffers=%zu first=%p/%u\n",
	       camera_format.width, camera_format.height, camera_format.pitch,
...
int zephyr_dual_display_init(void)
{
	...
	printk("dual-tflm: Zephyr MW405 ready fb=%p/%zu\n", fb.fb_addr, fb.fb_size);
	return start_camera();
}
